  ...
  return (
    <>
      <CategoryHero
        ...
        backgroundImage="https://images.unsplash.com/photo-1478720568477-152d9b164e26?w=1500&q=80"
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1fr_320px]">
          <div>
            <Breadcrumbs
              items={[{ label: 'Home', href: '/' }, { label: 'Entertainment' }]}
              className="mb-6"
            />

            <h1 className="mb-8 text-3xl font-bold text-gray-900 sm:text-4xl">Entertainment</h1>
            ...
            <nav className="mt-12 flex items-center justify-center gap-1" aria-label="Pagination">
              {/* Previous */}
              <Link
                href={currentPage > 1 ? `/category/entertainment?page=${currentPage - 1}` : '#'}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <Link
                  key={page}
                  href={`/category/entertainment?page=${page}`}
              ...
              {/* Next */}
              <Link
                href={currentPage < totalPages ? `/category/entertainment?page=${currentPage + 1}` : '#'}
                className={`inline-flex h-9 w-9 items-center justify-center rounded-lg text-sm transition-colors ${
